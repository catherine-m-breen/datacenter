...
#         join='override', # Safe here because monthly/daily slices have matching lat/lon shapes
#         parallel=False,  
#         engine='netcdf4'
#     )
...
#     # BUG FIX: Use combine='nested' instead of 'by_coords', and strictly remove join='override' 
#     # to avoid the leap-year size mismatch error.
...
#     # FIX FOR OVERLAPPING EDGES: Remove duplicate dates created at boundaries due to timezone/hour shifting
...
#     # TEXAS (Only running for Texas as requested)
#     process_state_full(
...
def process_state_full(base_path, output_dir, lat_slice, lon_slice, prefix, tz_name, utc_offset):
    ...
    for y_dir in year_dirs:
        ...
        if os.path.exists(out_filename):
            ...
            yearly_files.append(out_filename)
            continue
    ...
    print(f"Stitching {len(yearly_files)} yearly files into final master file...")
    # Open the intermediate files (perfectly safe and fast!)
...
if __name__ == "__main__":
    ...
    # TEXAS (Only running for Texas so we can finish it)
    process_state_full(
        base_path=base_forcing_path,
        output_dir='/discover/nobackup/cmbreen/datacenters/texas_hourly',
        lat_slice=slice(32.3, 33.3),
